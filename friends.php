    ...
    <div class="container-fluid mt-5">
        <div class="row">
            ...
            <div class="col-md-3 fixed-column">
                <div class="card">
                    ...
                    <?php
                        ...
                        <hr/ style="color:red;">
                        <!-- Friends -->
                        <h5 class="mt-4"><i class="fas fa-users"></i><a href="friends.php" class="link" style="color:#ff7c80;"> Friends</a> <i class="fas fa-arrow-left"></i></h5>
                        <hr/ style="color:red;">
                        <!-- About Us -->
            ...
            <div class="col-md-3 fixed-column">
                <div class="card">
                    <div class="card-body">
                        <h5 class="mt-4 friends">All Friends</h5>
                        <?php
                        require_once 'Database.php';
                        ...
                        $stmt = $pdo->query("SELECT id, first_name, last_name, role, profile_picture FROM users ORDER BY id DESC");
                        $users = $stmt->fetchAll(PDO::FETCH_ASSOC);

                        if (count($users) > 0) {
                            echo '<ul>';
                            foreach ($users as $user) {
                                $profilePicture = $user['profile_picture'] ? 'uploads/' . htmlspecialchars($user['profile_picture']) : 'uploads/default_profile_picture.jpg';

                                // Fetch the user role
                                $role = htmlspecialchars($user['role']);

                                echo '<a href="javascript:;" class="user-link" data-userid="' . $user['id'] . '" data-firstname="' . htmlspecialchars($user['first_name']) . '" data-lastname="' . htmlspecialchars($user['last_name']) . '">';
                                echo '<img src="' . $profilePicture . '" alt="Profile Picture" class="profile-picture">';
                                echo '<span>' . htmlspecialchars($user['first_name'] . ' ' . $user['last_name']) . '</span>';
                                
                                // Display role as badge
                                echo '<span class="badge ' . ($role === 'admin' ? 'bg-danger' : 'bg-info') . '">'. ucfirst($role) . '</span>';
            ...
            <div class="card">
                <div class="card-body">
                    <h5 class="mt-4" id="selected-user-name">Chat</h5>
